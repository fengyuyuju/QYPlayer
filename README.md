...
项目采用 **GetX MVC/MVVM 混合架构**：

- **Model 层**：`lib/bean/` 目录下的数据类，使用 json_serializable 进行序列化
- **View 层**：`lib/page/` 和 `lib/widget/` 中的 UI 组件，使用 Obx 响应状态变化
- **Controller 层**：`lib/controller/` 中的 GetxController，管理业务逻辑
- **State 层**：`lib/state/` 中的单例状态类（GlobalState、AudioState）
- **Service 层**：`lib/service/` 中的音频处理服务

## 目录结构

```
lib/
├── bean/           # 数据模型（42个 JSON 序列化实体）
├── controller/     # 业务逻辑控制器（11个文件）
├── state/          # 全局单例状态（GlobalState、AudioState）
├── page/           # 页面组件（9个页面）
├── widget/         # 可复用 UI 组件（24个组件）
├── service/        # 服务层（BookAudioHandler）
├── util/           # 工具函数（12个文件）
├── anim/           # 动画定义
├── l10n/           # 国际化资源
├── generated/      # 代码生成输出
